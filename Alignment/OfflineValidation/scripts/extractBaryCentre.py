...
def main():
    ...
    parser.add_argument('-p', '--partition', default='BPIX', help='Tracker partition (es. BPIX, FPIX, BPIXLYR1, ...)')
    parser.add_argument('-l', '--list'         , action='store_true', dest='list_entries', help='Just list the contents of file')
    parser.add_argument(      '--list-branches', action='store_true', help='Just list the branches of the tree')
    ...
    parser.add_argument('-s', '--style'    , default='twiki', choices=('twiki', 'latex', 'csv'), type=str.lower, help='Table style for the output')
    ...
    tree_begin = 'PixelBarycentre'         if args.type == 'barycentre' else 'BeamSpot'
    tree_name  = tree_begin if args.label is None else '_'.join([tree_begin, args.label])
...
def list_entries(fname):
    with TFileContext(fname) as tfile:
        for tfolder in tfile.GetListOfKeys():
            ...
            for tree in tfolder.ReadObj().GetListOfKeys():
                print('\t%-16s: %d entries in the TTree' %(tree.GetName(), tree.ReadObj().GetEntries()))
